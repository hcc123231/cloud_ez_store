...
//由于这是在多线程环境下，所以任何线程都有可能使用CachePool，需要加锁
class CachePool {
  public:
    // db_index和mysql不同的地方
    ...
    // Pool回收连接资源，本质上是将CacheConn放入到free_list_中
  ...
  private:
    string pool_name_;
    string server_ip_;//redis-server绑定的ip
    string password_;
    int m_server_port;//redis-server绑定的port
    int db_index_; // mysql 数据库名字， redis db index

    int cur_conn_cnt_;//当前维护的总CacheConn数量
    int max_conn_cnt_;//最大维护的总CacheConn数量
    list<CacheConn *> free_list_;//空闲CacheConn
...
class CacheManager {
  ...
  private:
    ...
    map<string, CachePool *> m_cache_pool_map;//记录名字对应的CachePool*
