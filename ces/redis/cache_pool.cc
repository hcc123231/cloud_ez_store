...
//创建CacheConn并且初始化它，然后入队到空闲连接队列中
int CachePool::Init() {
...
//读取配置文件中cacheDb相关配置，然后根据配置创建CachePool
...
//通过CachePool名字获取到CachePool实例，再通过CachePool对象获取CacheConn
...
//通过CacheConn中记录的CachelPool名字找到CachePool对象，然后通过CachePool对象释放CacheConn
